...
public struct BrowserGuideHostService: Sendable {
    ...
    private func execute(_ request: HostRequest) async throws -> [String: Any] {
        ...
        case .complete(let messagesData) where request.type == .complete:
            guard let anthropicClient, let importer else { throw engineUnavailable(request) }
            guard let accessToken = try importer.freshAnthropicAccessToken(now: Date()) else {
                throw HostFailure(
                    code: .notConfigured,
                    message: "Connect your Claude Code sign-in first — the Claude engine answers with your own token.",
                    retryable: false,
                    requestID: request.requestID
                )
            }
            do {
                let result = try await anthropicClient.complete(messagesData: messagesData, accessToken: accessToken)
                guard let content = try? JSONSerialization.jsonObject(with: result.contentJSON) else {
                    throw RealtimeClientError.invalidResponse
                }
                return ["content": content, "stopReason": result.stopReason]
            } catch let error as RealtimeClientError {
    ...
    private func engineUnavailable(_ request: HostRequest) -> HostFailure {
        HostFailure(
            code: .internalError,
            message: "The Claude engine is unavailable in this host build.",
            retryable: false,
            requestID: request.requestID
        )
    }
